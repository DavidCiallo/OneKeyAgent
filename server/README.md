...
代价：副本上发起的充值单会作为 put 回传主库、由主库确认入账，但**副本本地的
余额视图要等下次重启/重新引导才更新**。所以网页端的充值入口建议指向主库，
副本主要承担中转流量。
...
- **参考数据**：副本进程内缓存 30 秒。管理员在主库改动后，副本最长 30 秒可见；
  ...
  要等 TTL。
- **余额**：副本本地即时生效，主库按批延迟汇总，因此主库余额**会略滞后**；
  短暂超支是可接受的（副本扣到 0 就停）。
- **审计日志**：留在各节点本地，不上传主库。
- **周限额**：按主库的 `1d` 桶统计，副本侧同样滞后于批量推送，属已知取舍。
- 反向同步（主库 → 副本）目前只在副本启动引导时发生；副本运行期间不轮询主库。
  若要让副本立刻看到主库的新模型，重启副本或等主库成为唯一写入点时再改。
